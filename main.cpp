...
int main(int argc, char* argv[])
{
    ...
    cin >> ch1;
    ifstream file_txt;
    file_txt.open("my_text4.txt", ios::in);
...
for (size_t i=0; i<vector_of_strings.size(); ++i)
    {
        ...
        for(size_t j =0; j<string_from_file.length(); ++j)
            {
             if (tolower(ch1) == tolower(string_from_file[0]))

                        {
                            k=k+1;
                            vector_sorted.insert(vector_sorted.end(), string_from_file);
                             break;
                        }
            }
    }
sort(vector_sorted.begin(), vector_sorted.end(), greater<>());
for(auto m : vector_sorted)
cout << "Sorted words " << m << endl;
